        ...
        with P4Connection() as p4_conn:
            ...
            for cl_num in stack_to_process[1:]:
                ...
                if parent_cl is None:  # Should be impossible if stack is correct
                    log.error(
                        f"Logic error: CL {cl_num} in stack but has no parent. Aborting."
                    )
                ...
                merged_result = three_way_merge_folder(
                    base_folder, ours_folder, theirs_folder
                )
                ...
                while conflicted_files:
                    try:
                        if not typer.confirm(
                            "\nConflict detected, press ENTER to resolve... (or 'n' to abort)",
                            default=True,
                        ):
                    ...
                    for file_path, content in resolved_files.items():
                        if (
                            ...
                            console.print(
                                f"Conflict markers still detected in: {file_path}"
                            )
                            conflicted_files[file_path] = content
                        else:
                            merged_snapshot[file_path] = content
